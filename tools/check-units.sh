...
#
# Inside the phone (src/app, src/buyer) every length must be in px:
...
HITS=$(grep -rnE '[0-9](rem|vw|vh|dvh|svh|lvh|dvw|svw)\b' src/app src/buyer || true)
...
echo "unit check passed: no browser-relative units under src/app or src/buyer"
